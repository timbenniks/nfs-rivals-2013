...
(function ($, ns, controller, tween)
{
	...
	controller.intro = function ()
	{
		var event = ns.EventHandlerInstance,
			...
			leftBtnIconEl, rightBtnIconEl, 
			leftBtnTextEl, rightBtnTextEl,
			headerH1, headerH2, leftCanvas, rightCanvas,
			choiceContentWrapperEl, choiceVideoWrapperEl, videoEl,
			leftCanvasInstance, rightCanvasInstance, stopEl,
			ease = Expo.easeInOut,
			speed = 0.5,
		...
		buildUpScreen = function ()
		{
			...
			tween.to(rightCanvas[0], speed, { left: 0, ease: ease, onComplete: function ()
				{
					...
					setTimeout(function ()
					{
						ns.Controller.destroy($('#preloader'), function () { $('#preloader').remove(); });
					}, 1000);		
				}
			});
		...
		bindEvents = function ()
		{
			...
			leftChoiceEl.on('mouseenter', function () { handleEnter('left', this); });
			rightChoiceEl.on('mouseenter', function () { handleEnter('right', this); });
			
			leftChoiceEl.on('click', function () { handleClick('left', this); });
			rightChoiceEl.on('click', function () { handleClick('right', this); });
		...
		handleClick = function (side)
		{
			...
			if(side === 'left')
			{
				...
				tween.to(leftBtnEl[0], speed, { left: '350px', opacity: 0, ease: ease });
				tween.to(rightBtnEl[0], speed, { right: '-1100px', opacity: 0, ease: ease });
				...
				player.addEventListener('timeupdate', function (e)
				{
				    ...
				    if (newPosition % videoEventInterval < (videoPosition % videoEventInterval))
				    {
				        ...
				        {
				            event.introTvcPlaying(Math.floor(newPosition / videoEventInterval) * videoEventInterval);
				        }
				    }
		...
		hideHeaders = function(side)
		{
			...
			tween.to(headerH1[0], speed * 1.1, { left: (side === 'left') ? '1100px' : '-1100px', ease: ease });
			tween.to(headerH2[0], speed, { left: (side === 'left') ? '1100px' : '-1100px', ease: ease });
		...
		setBtnPos = function (side)
		{
			...
			if(side === 'left')
			{
				...
				tween.to(leftBtnTextEl[0], speed * 1.2, { opacity: 1, left: '-20px', ease: ease });
			}
			...
			if(side === 'right')
			{
				...
				tween.to(rightBtnTextEl[0], speed * 1.2, { opacity: 1, left: '20px', ease: ease });
			}
		},
		...
		resetBtnPos = function (side)
		{
			if(side === 'left')
			{
				tween.killTweensOf(leftBtnEl[0]);
				tween.killTweensOf(leftBtnTextEl[0]);
				tween.to(leftBtnEl[0], speed, { left: leftBtnPos, opacity: 1, ease: ease });
				tween.to(leftBtnTextEl[0], speed, { opacity: 0, left: '0px', ease: ease });
			}
			...
			if(side === 'right')
			{
				tween.killTweensOf(rightBtnEl[0]);
				tween.killTweensOf(rightBtnTextEl[0]);
				tween.to(rightBtnEl[0], speed, { right: rightBtnPos, opacity: 1, ease: ease });
				tween.to(rightBtnTextEl[0], speed, { opacity: 0, left: '60px', ease: ease });
			}
		},
