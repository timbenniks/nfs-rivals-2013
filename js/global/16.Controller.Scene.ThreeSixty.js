  ...
  controller.scenethreesixty = function (params) {
    var event = ns.EventHandlerInstance,
      ...
      pathPoints = {
        ...
        topLeftY: 0,
        bottomLeftX: 0,
        bottomLeftY: ns.height,
        topRightX: 0,
        topRightY: 0,
        bottomRightX: 0,
        bottomRightY: ns.height,
      },
      init = function () {
        controller.getAssets(data.assets["threesixty"], {
          onComplete: function () {
            threesixtyComplete = true;
          },
        });
        controller.getAssets(data.assets["threesixty_hotspots"], {
          onComplete: function () {
            threesixtyHotspotsComplete = true;
          },
        });
      ...
      onAssetsLoadedComplete = function () {
        ...
        $.each(data.hotspots, function (i, e) {
          var hotspotDataObject = {
            ...
            subtitle: data.copy[e.id + "_subtitle"]
              ? data.copy[e.id + "_subtitle"]
              : false,
            title: data.copy[e.id + "_title"]
              ? data.copy[e.id + "_title"]
              : false,
            description: data.copy[e.id + "_description"]
              ? data.copy[e.id + "_description"]
              : false,
          };

          if (e.id !== "hotspot_intro") {
            hotspotDataObject.image =
              ns.AssetPreloaderInstance.getCacheBustedUrl(
                ns.BaseUrl +
                  "scene_" +
                  scene +
                  "_threesixty_hotspots/scene_" +
                  scene +
                  "_threesixty_" +
                  e.id +
                  ".jpg"
        ...
        introData.image = ns.AssetPreloaderInstance.getCacheBustedUrl(
          ns.BaseUrl +
            "scene_" +
            scene +
            "_threesixty_hotspots/scene_" +
            scene +
            "_threesixty_hotspot_0.jpg"
        );
        introData.copy1 = data.copy["intro_copy1"];
        introData.copy2 = data.copy["intro_copy2"];

        var tmplHtml = tmplSelector.html(),
          tmpl = controller.getTmpl(tmplHtml, {
            scene: scene,
            intro: introData,
      ...
      onTemplateRendered = function () {
        ...
        showIntro(function () {
          ...
          controller.destroy($("#scene_choice"), function () {
            $("#scene_choice").removeClass("active").empty();
          });
        });
      },
