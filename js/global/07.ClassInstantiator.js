  ...
  ns.ClassInstantiator = function (dataAttr) {
    var dataAttributeToScanFor = dataAttr || "widget",
      ...
      findWidgets = function (context) {
        ...
        var widgetsFound = getNode(
            context,
            "[data-" + dataAttributeToScanFor + "]"
          ),
          i = 0,
          c,
          widgets = [];
        ...
        for (; i < widgetsFound.length; i++) {
          var widget = widgetsFound[i],
            widgetToInstantiate = getDataAttr(
              widget,
              dataAttributeToScanFor
            ).split(";");

          for (c = 0; c < widgetToInstantiate.length; c++) {
            widgets.push({
              widget: widget,
              widgetToInstantiate: widgetToInstantiate[c],
            });
          }
        }
      ...
      instantiate = function (node, widgetName) {
         ...
         */
        options =
          getDataAttr(node, widgetName.toLowerCase() + "Options") ||
          getDataAttr(node, "options") ||
          {};
      ...
      cleanUpInstanceName = function (widgetName) {
        ...
        switch (parts.length) {
          ...
          case 3:
            return ns[parts[0]][parts[1]][parts[2]];
          case 4:
            return ns[parts[0]][parts[1]][parts[2]][parts[3]];
          default:
            throw (
              "Cannot find [" +
              widgetName +
              "] as a function in your namespace."
            );
        }
      },
      /**
      ...
      destroyWidgetsInContext = function (context, widgetName) {
        ...
        if (widgetName) {
          widgetsFound = getNode(
            context,
            "[data-" + dataAttributeToScanFor + '="' + widgetName + '"]'
          );
        } else {
          widgetsFound = getNode(
            context,
      ...
      getWidgetFromDict = function (node, widgetName) {
        for (var i = 0; i < widgetDictionaryLength; i++) {
          if (
            widgetDictionary[i].node === node &&
            widgetDictionary[i].widgetName === widgetName
          ) {
      ...
      setWidgetInDict = function (node, widgetName, instance) {
        ...
        widgetDictionary[widgetDictionaryLength++] = {
          node: node,
