  ...
  controller.scenesetup = function (params) {
    var event = ns.EventHandlerInstance,
      ...
      init = function () {
        ...
        if (scene === 1) {
          instructionOneStart = 10;
          instructionOneEnd = 12.5;
          instructionTwoStart = 14;
          instructionTwoEnd = 18;
        }
        if (scene === 2) {
          instructionOneStart = 3.7;
          instructionOneEnd = 6.2;
          instructionTwoStart = 6.5;
          instructionTwoEnd = 8.3;
        }
        if (scene === 3) {
          instructionOneStart = 0;
          instructionOneEnd = 2.5;
          instructionTwoStart = 5.5;
          instructionTwoEnd = 7;
        }
        ...
        $.each(setupAssets, function () {
          ...
          if (splittedSrc[1] === "jpg") {
            tmplData.poster =
              ns.BaseUrl +
              ns.AssetPreloaderInstance.getCacheBustedUrl(this.src);
          } else {
      ...
      getChoiceAssetsForScene = function () {
        controller.getAssets(
          controller.getData("scene_" + scene + "_choice").assets["left"],
          {}
        );
        controller.getAssets(
          controller.getData("scene_" + scene + "_choice").assets["right"],
          {}
        );
      },
