  ...
  controller.end = function () {
    var event = ns.EventHandlerInstance,
      ...
      init = function () {
        ...
        for (
          var i = 1;
          i < parseInt(controller.getData("intro").misc.subtitle_count) + 1;
          i++
        ) {
          subs.push({
            start:
              controller.getData("intro").copy["subtitles_" + i + "_start"],
            end: controller.getData("intro").copy["subtitles_" + i + "_end"],
            text: controller.getData("intro").copy["subtitles_" + i + "_text"],
          });
        }
        ...
        controller.renderTmpl(htmlWrapper, tmpl, function () {
          ...
          outroVideoEl[0].addEventListener(
            "ended",
            function () {
