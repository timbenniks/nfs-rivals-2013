...
(function ($, ns, mediaElementPlayer, popcorn)
{
	...
	ns.VideoPlayer = function (el, options)
	{
		...
		init = function ()
		{
			player = new mediaElementPlayer(el,
			{
				...
				success: function (mediaElement)
				{
					...
					$(el).parents('.mejs-container').next('.video-overlay').find('button').off('click', handleIconClick);
					$(el).parents('.mejs-container').next('.video-overlay').find('button').on('click', handleIconClick).on('mouseenter', handleIconEnter);
					...
					{
						if (controls) $(el).parents('.mejs-container').find('.mejs-controls').show();
						$(el).parents('.mejs-container').next('.video-overlay').fadeOut(400);
					});
					...
					if (controls)
					{
						...
						{
							$(el).parents('.mejs-container').next('.video-overlay').fadeIn(400);
						});
					...
					if (poster)
					{
						$(el).after('<div id="poster"><img src="' + poster + '" /></div>');
					}
		...
		destroy = function ()
		{
			if(mejs.players[$(el).parents('.mejs-container').attr('id')]) mejs.players[$(el).parents('.mejs-container').attr('id')].remove();
			$(el).parents('.mejs-container').next('.video-overlay').find('button').off('click', handleIconClick).off('mouseenter', handleIconEnter);
		};
